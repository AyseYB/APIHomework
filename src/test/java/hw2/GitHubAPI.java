...
//5. Verify value of the id field is 320565
    ...
    public void organizationInfo(){
        ...
        response.then().statusCode(200).contentType("application/json; charset=utf-8").
                ...
                body("name", equalToIgnoringCase("cucumber")).// or we can use equalToIgnoreCase instead of is
                body("id", is(320565));
...
//4. Verify that number of objects in the response is equal to value from step 2
    ...
    public void numberOfRepositories(){
        ...
        Response response1 = given().
                queryParam("per_page", 100).  //<-- since it shows only 30 repo per page we wrote 100
                pathParams("org", "cucumber").
                when().
                get("/orgs/{org}/repos").prettyPeek();
        response1.then().assertThat().body("size()", is(numberOfRepositories));
    }
...
//4. Verify that value of the id inside the owner object in every response is equal to value from step 2
    ...
    public void repoOwnerInfo() {
        ...
        Response response1 = given().queryParam("per_page", 100).
                pathParams("org", "cucumber").
                when().
                get("/orgs/{org}/repos").prettyPeek();

        response1.then().
                assertThat().body("owner.id", everyItem(is(id)));
